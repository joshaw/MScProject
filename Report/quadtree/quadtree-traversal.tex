...
all $\rfrac{1}{9}$'s can be used. The sum of the nine elements is one, meaning
the overall brightness of the image is not changed. When applied, this would
mean that the current pixel, $p$, is given a value which is the sum of each of
matrix elements, $m_i = \rfrac{1}{9}$, multiplied by the pixel value beneath
it, $v_i$, Equation~\ref{eq:pixel_matrix}.

\begin{align}
	& s := \texttt{kernel.size} \notag \\
	& \texttt{for each pixel p in image}: \notag \\
	& \qquad\qquad p := \sum_{i=0}^s m_i * v_i \label{eq:pixel_matrix}
\end{align}
...
\begin{figure}[tbh]
	\centering
	\includegraphics[width=6cm]{kernel-shapes.pdf}
